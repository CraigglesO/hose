...
test("Handshake with Buffers", function (t) {
  ...
  let hose = new Hose(infoHash, idBuf);
  hose.on("error", (err) => { t.fail(err.toString()); });
  hose.pipe(hose);
...
test("Bad Handshake", function (t) {
  ...
  let hose  = new Hose(infoHash, idBuf);
  let hose2 = new Hose(infoHash2, idBuf);
  hose.on("error", (err) => { t.fail(err.toString()); });
...
test("Extended Handshake without options", function (t) {
  ...
  let hose = new Hose("e940a7a57294e4c98f62514b32611e38181b6cae", id);
  let hose2 = new Hose("e940a7a57294e4c98f62514b32611e38181b6cae", id);
  hose.on("error", (err) => { t.fail(err.toString()); });
  hose2.on("error", (err) => { t.fail(err.toString()); });
...
test("Extended Handshake with options", function (t) {
  ...
  let hose = new Hose("e940a7a57294e4c98f62514b32611e38181b6cae", id);
  let hose2 = new Hose("e940a7a57294e4c98f62514b32611e38181b6cae", id, opts);
  hose.on("error", (err) => { t.fail(err.toString()); });
  hose2.on("error", (err) => { t.fail(err.toString()); });
  hose.pipe(hose2).pipe(hose);
